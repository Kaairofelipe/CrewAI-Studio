...
# Cursor hook: beforeShellExecution — só corre em `git commit` (ver hooks.json).
# Devolve apenas JSON em stdout.
#
# Regras (apenas se existir ./convex/ no workspace):
# - Bloqueia Date.now() perto de query({}) em Convex.
# - Bloqueia .filter() encadeado a db.query().
#
# Sem pasta convex/, o script permite sempre (CrewAI-Studio / projetos não-Convex).
...
# Script em .cursor/hooks/ → raiz do repo é dois níveis acima
